...
jsx.Location = new function(){
    ...
    // TODO: find places where I use it and use getDelimiter
    ...
    this.getParam = function(name, location){
        ...
        var param = location.match(new RegExp(name + '=[^&^$]*', 'ig'));
        if (!param || !param.length){
            return '';
        }
        return param[0].replace(new RegExp(name + '=', ''), '');
    };

    this.setParam = function(name, value,  location){
        location = location || window.location.search;
        if (this.getParam(location)){
          location.replace(new RegExp(name + '=[^&^$]*', 'ig'), name + '=' + value);
        }else{
