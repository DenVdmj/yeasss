...
/* generic case */
				default:
					var doc = document,
						isSingle = !/\s|,/.test(selector);
/* return some simple cases: only ID or only CLASS */
					switch (selector[0]) {
						case '#':
							if (isSingle) {
								_.sets = _.getUniqueId(doc, selector.slice(1));
								break;
							}
						case '.':
							if (isSingle && root.getElementsByClassName(selector.slice(1))) {
								_.sets = root.getElementsByClassName(selector.slice(1));
								break;
							}
/* all other cases */
						default:
...
/* loop in siblings back while nodeType isn't element */
...
/* put selected nodes in local nodes' set */
												_.nodes = newNodes ? newNodes : null;
												break;
										}
									});
								}
...
_.cache = {};
/* method to return unique element by ID */
_.getUniqueId = function(doc, id) {
...
*/
	if (doc.all && node.id !== id && (nodes = doc.all[id])) {
